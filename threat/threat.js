...
async function fetchWCLreport(path, start, end) {
  ...
  let filter = encodeURI(
    `type IN ("death","cast","begincast") OR ability.id IN (${Object.keys(
      ...
      ','
    )}) OR (type IN ("damage","heal","miss","applybuff","applybuffstack","refreshbuff","applydebuff","applydebuffstack","refreshdebuff","energize","absorbed","healabsorbed","leech","drain") AND ability.id NOT IN (${zeroThreatSpells.join(
      ','
    )}))`
  );
